...
export function validateInterviewProblem(problem: any): boolean {
  ...
  if (!validTypes.includes(problem.type)) {
    console.warn(
      `Interview problem validation failed: invalid type ${problem.type}`
    );
    return false;
  }
  ...
  if (!validDifficulties.includes(problem.difficulty)) {
    console.warn(
      `Interview problem validation failed: invalid difficulty ${problem.difficulty}`
    );
    return false;
  }
  ...
  if (!problem.problem || typeof problem.problem !== 'object') {
    console.warn(
      'Interview problem validation failed: missing or invalid problem object'
    );
    return false;
  }
  ...
  for (const field of requiredProblemFields) {
    if (problemObj[field] === undefined || problemObj[field] === null) {
      console.warn(
        `Interview problem validation failed: missing problem.${field}`
      );
      return false;
    }
  }
...
export function validateSimulationProblem(problem: SimulationProblem): boolean {
  ...
  if (!validTypes.includes(problem.type)) {
    console.warn(
      `Simulation problem validation failed: invalid type ${problem.type}`
    );
    return false;
  }
  ...
  if (!validDifficulties.includes(problem.difficulty)) {
    console.warn(
      `Simulation problem validation failed: invalid difficulty ${problem.difficulty}`
    );
    return false;
  }
...
function validateSimulationProblemContent(problem: SimulationProblem): boolean {
  ...
  switch (problem.type) {
    ...
    default:
      console.warn(
        `Simulation problem validation failed: unknown type ${problem.type}`
      );
      return false;
  }
}
...
function validateDSAContent(content: any): boolean {
  ...
  if (!Array.isArray(content.examples) || content.examples.length === 0) {
    console.warn(
      'DSA content validation failed: examples must be a non-empty array'
    );
    return false;
  }
...
function validateMachineCodingContent(content: any): boolean {
  ...
  if (!hasRequirements && !hasAcceptanceCriteria && !hasTechnologies) {
    console.warn(
      'Machine coding content validation failed: missing requirements, acceptanceCriteria, or technologies'
    );
    return false;
  }
...
function validateSystemDesignContent(content: any): boolean {
  ...
  const hasFunctionalRequirements =
    Array.isArray(content?.functionalRequirements) &&
    content.functionalRequirements.length > 0;
  const hasNonFunctionalRequirements =
    Array.isArray(content?.nonFunctionalRequirements) &&
    content.nonFunctionalRequirements.length > 0;
  const hasScale = content?.scale && typeof content.scale === 'object';
  const hasExpectedDeliverables =
    Array.isArray(content?.expectedDeliverables) &&
    content.expectedDeliverables.length > 0;
  ...
  if (validFields.length < 2) {
    console.warn(
      'System design content validation failed: insufficient required fields'
    );
    return false;
  }
...
function validateTheoryContent(content: any): boolean {
  ...
  const hasQuestion =
    ...
    content.question.trim().length > 0;
  const hasExpectedAnswer =
    content?.expectedAnswer &&
    typeof content.expectedAnswer === 'string' &&
    content.expectedAnswer.trim().length > 0;
  const hasKeyPoints =
    Array.isArray(content?.keyPoints) && content.keyPoints.length > 0;

  // At least two of these should be present
  const validFields = [hasQuestion, hasExpectedAnswer, hasKeyPoints].filter(
    Boolean
  );
  if (validFields.length < 2) {
    console.warn(
      'Theory content validation failed: insufficient required fields'
    );
    return false;
  }
...
export function logValidationResults(
  problems: any[],
  validationFunction: (problem: any) => boolean,
  context: string
) {
  ...
  if (invalidProblems.length > 0) {
    ...
    invalidProblems.forEach((problem, index) => {
      console.log(
        `  ${index + 1}. ${problem.title || 'Untitled'} (${problem.type || 'unknown type'})`
      );
    });
  }
