...
import { saveInterviewProblemDocument } from '../firebase/problems';
...
function validateDSAProblem(problem: SimulationProblem): boolean {
  ...
  const hasProblemStatement =
    content?.problemStatement && typeof content.problemStatement === 'string';
...
function validateMachineCodingProblem(problem: SimulationProblem): boolean {
  ...
  if (!hasRequirements && !hasAcceptanceCriteria && !hasTechnologies) {
    console.warn(
      'Machine coding problem validation failed: missing requirements, acceptanceCriteria, or technologies'
    );
    return false;
  }
...
function validateSystemDesignProblem(problem: SimulationProblem): boolean {
  ...
  if (validFields.length < 2) {
    console.warn(
      'System design problem validation failed: insufficient required fields'
    );
    return false;
  }
...
function validateTheoryProblem(problem: SimulationProblem): boolean {
  ...
  if (validFields.length < 2) {
    console.warn(
      'Theory problem validation failed: insufficient required fields'
    );
    return false;
  }
  ...
  for (let i = 0; i < rounds.length; i++) {
    ...
    for (let j = 0; j < problemsCount; j++) {
      try {
        ...
        console.error(
          `Error generating problem ${j + 1} for round ${i + 1}:`,
          error
        );
      }
    }
 ...
 */
export const saveGeneratedProblemsToCollection = async (
  ...
  try {
    ...
    console.error('Error in saveGeneratedProblemsToCollection');
...
export const createInterviewSimulation = async (
  params: any // CreateSimulationParams is removed, so using 'any' for now
): Promise<string> => {
  ...
  try {
    ...
    const payload: Partial<any> & { rounds: any[] } = {
      // InterviewSimulationData is removed, so using 'any'
      userId,
      companyName,
      roleLevel,
