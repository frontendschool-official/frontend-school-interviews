...
import { GEMINI_ENDPOINT, getGeminiApiKey, getPromptVersion } from "./gemini-config";
import {
  ProblemSchema,
  GenerateParams,
  GeneratedResult,
      ...
      switch (interviewType) {
        case INTERVIEW_TYPE.DSA:
          ...
          if (!isValidDSA) {
            ...
            console.error("❌ DSA problem validation failed:", parsedResponse.problem);
          }
          break;
        ...
        case INTERVIEW_TYPE.THEORY:
          ...
          if (!isValidTheory) {
            ...
            console.error("❌ Theory problem validation failed:", parsedResponse.problem);
          }
          break;

        case INTERVIEW_TYPE.MACHINE_CODING:
          const isValidMachine = isValidMachineCodingProblem(parsedResponse.problem);
          if (!isValidMachine) {
            validationPassed = false;
            validationErrors.push("Machine coding problem validation failed");
            console.error("❌ Machine coding problem validation failed:", parsedResponse.problem);
          }
          break;

        case INTERVIEW_TYPE.SYSTEM_DESIGN:
          const isValidSystem = isValidSystemDesignProblem(parsedResponse.problem);
          ...
          if (!isValidSystem) {
            ...
            console.error("❌ System design problem validation failed:", parsedResponse.problem);
          }
          break;

        default:
          // For combined problems, try to validate as any type
          const isValidAny = isValidDSAProblem(parsedResponse.problem) ||
          ...
          if (!isValidAny) {
            validationPassed = false;
            validationErrors.push("Combined problem validation failed - no valid problem type detected");
            console.error("❌ Combined problem validation failed:", parsedResponse.problem);
          }
          break;
      }
    }
