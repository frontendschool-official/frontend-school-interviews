...
import { RoadmapDocument, RoadmapProgress } from "../../types/roadmap";
 ...
 */
export const saveRoadmap = async (roadmapData: Omit<RoadmapDocument, "id" | "createdAt" | "updatedAt">) => {
...
export const getRoadmapsForUser = async (userId: string) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`✅ Found ${roadmaps.length} roadmaps for user (with ordering)`);
      return roadmaps;
      ...
      if (indexError.message && indexError.message.includes("index")) {
        ...
        console.log(`✅ Found ${roadmaps.length} roadmaps for user (sorted in memory)`);
        return roadmaps;
      } else {
        throw indexError;
      }
...
export const getRoadmapStats = async (userId: string) => {
  ...
  try {
    ...
    const stats = {
      ...
      totalProblems: roadmaps.reduce((sum, r) => sum + (r.overview.totalProblems || 0), 0),
      completedProblems: roadmaps.reduce((sum, r) => sum + (r.progress?.completedProblemsCount || 0), 0),
      averageProgress: 0,
    };

    // Calculate average progress
    const roadmapsWithProgress = roadmaps.filter(r => r.progress && r.overview.totalProblems > 0);
    if (roadmapsWithProgress.length > 0) {
      const totalProgress = roadmapsWithProgress.reduce((sum, r) => {
        return sum + ((r.progress?.completedProblemsCount || 0) / r.overview.totalProblems);
      }, 0);
      stats.averageProgress = Math.round((totalProgress / roadmapsWithProgress.length) * 100);
    }
