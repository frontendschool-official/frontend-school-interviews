...
// Fetch all problems from the unified interview_problems collection
export const getAllProblems = async (userId?: string) => {
  try {
    const ref = collection(db, 'interview_problems');
    ...
    const allProblems = snapshot.docs.map((d) => {
      ...
      return {
        id: d.id,
        ...data,
        // Determine type based on interviewType field or existing type field
