...
if (missingConfig.length > 0) {
  ...
  console.error('Please create a .env.local file with your Firebase project credentials');
}
...
export type { MachineCodingProblem, SystemDesignProblem } from '../types/problem';
...
export const saveSubmission = async (userId: string, problemId: string, data: SubmissionData) => {
  if (!userId || !problemId) {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      throw new Error('An unexpected error occurred while saving your submission');
    }
  }
};
...
export const getSubmissionsForUser = async (userId: string) => {
  try {
    ...
    const q = query(ref, where('userId', '==', userId), orderBy('createdAt', 'desc'));
    const snapshot = await getDocs(q);
    console.log('Submissions snapshot retrieved, docs count:', snapshot.docs.length);
...
export const getProblemById = async (id: string): Promise<ParsedProblemData | null> => {
  console.log('id', id)
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      throw new Error('An unexpected error occurred while fetching the problem');
    }
  }
};
...
export const getAllProblems = async () => {
  try {
    ...
    console.log('Raw documents from interview_problems:', snapshot.docs.map(d => ({ id: d.id, ...d.data() })));
    ...
    const allProblems = snapshot.docs.map((d) => {
      ...
      const problem = {
        ...
        type: data.type || (data.interviewType === 'dsa' ? 'dsa' : 
                           data.interviewType === 'coding' ? 'machine_coding' : 
                           data.interviewType === 'design' ? 'system_design' : 'user_generated'),
        // Use existing category field if available, otherwise determine from type
        category: data.category || (data.interviewType === 'dsa' ? 'Data Structures & Algorithms' :
                                   data.interviewType === 'coding' ? 'Machine Coding' :
                                   data.interviewType === 'design' ? 'System Design' : 'Custom Problems')
      };
      console.log('Processed problem:', problem);
      return problem;
    });

    console.log('All Problems from interview_problems collection:', allProblems);
    return allProblems;
  } catch (error) {
