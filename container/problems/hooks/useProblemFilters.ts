...
export const useProblemFilters = ({
  ...
  const stats = useMemo(
    (): ProblemStats => ({
      ...
      machineCoding:
        problems?.filter(p => getProblemType(p) === 'machine_coding').length ||
        0,
      systemDesign:
        problems?.filter(p => getProblemType(p) === 'system_design').length ||
        0,
      theory:
        problems?.filter(p => getProblemType(p) === 'theory_and_debugging')
          .length || 0,
      attempted:
        user && statuses
  ...
  const filteredProblems = useMemo(() => {
    ...
    return problems.filter(problem => {
      ...
      if (activeProblemFilter === 'machine_coding')
        return problemType === 'machine_coding';
      if (activeProblemFilter === 'system_design')
        return problemType === 'system_design';
      if (activeProblemFilter === 'theory')
        return problemType === 'theory_and_debugging';
      if (activeProblemFilter === 'attempted')
        return statuses[problem.id || ''] === 'attempted';
      return true;
    });
