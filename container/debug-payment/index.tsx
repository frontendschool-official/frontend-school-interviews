...
const DebugPaymentContainer: NextPage = () => {
  ...
  const testPaymentFlow = async () => {
    ...
    try {
      ...
      addLog(`Payment verification result: ${JSON.stringify(verificationResult, null, 2)}`);
      ...
      addLog(`Payment saved to Firebase: ${JSON.stringify(savedPayment, null, 2)}`);
  ...
  const checkEnvironment = () => {
    ...
    requiredVars.forEach(varName => {
      ...
      if (value) {
        addLog(`✅ ${varName}: ${varName.includes('SECRET') ? '***' : value.substring(0, 10)}...`);
      } else {
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto p-8">
        <h1 className="text-3xl font-bold text-text mb-8">Payment Debug Tools</h1>
        
        <div className="bg-secondary border border-border rounded-2xl p-8 mb-8">
          <h3 className="text-text mb-4 text-xl font-semibold">Test Functions</h3>
          <button 
