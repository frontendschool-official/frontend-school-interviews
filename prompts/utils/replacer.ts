 ...
 */
export function createDefaultVariableMap(
  overrides: Partial<VariableMap> = {}
): VariableMap {
...
export class PromptVariableBuilder {
  ...
  setTechnology(tech: {
    ...
    if (tech.secondaryTechnology)
      this.variables.secondaryTechnology = tech.secondaryTechnology;
    if (tech.technologyStack)
      this.variables.technologyStack = tech.technologyStack;
  ...
  setProblemConfig(config: {
    ...
    if (config.estimatedTime)
      this.variables.estimatedTime = config.estimatedTime;
    if (config.category) this.variables.category = config.category;
    if (config.interviewType)
      this.variables.interviewType = config.interviewType;
    return this;
  }
