  ...
  <div className='mb-6 sm:mb-8'>
    ...
    <div className='grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8'>
      {[1, 2, 3, 4].map(index => (
        <div
          key={index}
          className='bg-secondary px-6 py-4 rounded-2xl border border-border text-center shadow-lg'
        >
    ...
    <div className='p-6 sm:p-8'>
      ...
      <div className='space-y-4'>
        {[1, 2, 3, 4, 5].map(index => (
          <div
            key={index}
            className='p-4 sm:p-5 rounded-2xl border-2 border-border'
          >
        ...
        <div>
          ...
          <div className='flex flex-wrap gap-2'>
            {[1, 2, 3, 4].map(index => (
              <div
                key={index}
                className='w-20 h-8 bg-border rounded-xl animate-pulse'
              ></div>
            ))}
          </div>
        </div>
        ...
        <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mt-6'>
          <div className='flex gap-2'>
            {[1, 2, 3].map(index => (
              <div
                key={index}
                className='w-20 h-10 bg-border rounded-lg animate-pulse'
              ></div>
            ))}
          </div>
...
const RoadmapViewPage: NextPage = () => {
  ...
  const filteredProblems = useMemo(() => {
    ...
    return (currentDay.problems as RoadmapProblem[]).filter(
      (_: RoadmapProblem, index: number) => {
  ...
  const resumeDay = useMemo(() => {
    ...
    for (const day of roadmap.dailyPlan) {
      ...
      const completed =
        day.problems?.filter((_: RoadmapProblem, index: number) =>
        ...
        <div className='mb-6 sm:mb-8'>
          ...
          <div className='grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8'>
            <div className='bg-secondary px-6 py-4 rounded-2xl border border-border text-center shadow-lg'>
              ...
              <div className='text-sm text-text opacity-70 font-medium'>
                Target Role
              </div>
            </div>
            <div className='bg-secondary px-6 py-4 rounded-2xl border border-border text-center shadow-lg'>
                          ...
                          items={[
                            {
                              id: 'all',
                              label: 'All',
                              count: currentDayStats.total,
                            },
                            {
                              id: 'incomplete',
                              label: 'Incomplete',
                              count: currentDayStats.incomplete,
                            },
                            {
                              id: 'completed',
                              label: 'Completed',
                              count: currentDayStats.completed,
                            },
                          ]}
                          activeTab={problemFilter}
                          onTabChange={tabId =>
                            setProblemFilter(tabId as typeof problemFilter)
                          }
                        />
                        <div className='flex items-center gap-2'>
                          <Button
                  ...
                  {filteredProblems.length === 0 && (
                    <Card className='border-2'>
                      <div className='p-8 text-center'>
                        ...
                        <h4 className='text-xl font-bold text-neutralDark mb-2'>
                          Nothing to show here
                        </h4>
                        <p className='text-text opacity-70'>
                          Try switching the filter or move to the next day.
                        </p>
                      </div>
                    </Card>
                  )}
                  {filteredProblems.map(
                    (problem: RoadmapProblem, index: number) => {
                      // Map index from filtered array back to original array index for completion state
                      const originalIndex =
                        currentDay?.problems?.indexOf(problem) ?? index;
