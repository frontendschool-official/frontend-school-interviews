...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const completedSimulations = allSimulations.filter(
      (sim: any) => sim.status === 'completed' || sim.status === 'evaluated'
    );
    const activeSimulations = allSimulations.filter(
      (sim: any) => sim.status === 'active' || sim.status === 'in-progress'
    );

    const totalInterviews =
      completedSimulations.length + activeSimulations.length;
    const completedRounds = completedSimulations.reduce(
      (total: number, sim: any) => {
    ...
    const totalTimeMinutes = completedSimulations.reduce(
      (total: number, sim: any) => {
        const startedAt = sim.startedAt?.toDate?.() || new Date(sim.startedAt);
        const completedAt =
          sim.completedAt?.toDate?.() || new Date(sim.completedAt);
    ...
    const recentActivity = completedSimulations
      ...
      .map((sim: any) => ({
        ...
        title: sim.companyName
          ? `${sim.companyName} Interview`
          : sim.title || 'Mock Interview',
        company: sim.companyName || 'Mock Interview',
        status: 'completed',
        score: sim.totalScore || sim.score,
        timeSpent: sim.timeSpent
          ? `${Math.floor(sim.timeSpent / 60)}m`
          : undefined,
