...
import { FiPlay, FiRotateCcw, FiClock, FiPause, FiCheck } from 'react-icons/fi';
  ...
  const handleNextProblem = () => {
    ...
    if (currentProblem < totalProblems - 1) {
      ...
      const result: MockInterviewResult = {
        ...
        problemEvaluations: evaluations.filter(
          Boolean
        ) as MockInterviewEvaluation[],
          ...
          <div className='flex items-center gap-3'>
            <div className='w-30 h-1 bg-gray-700 rounded overflow-hidden'>
              <div
                ...
                style={{
                  width: `${totalProblems ? ((currentProblem + 1) / totalProblems) * 100 : 0}%`,
                }}
              ></div>
            </div>
            <span className='text-sm text-gray-400'>
              {currentProblem + 1}/{totalProblems}
            </span>
          </div>
        </div>
      </div>
