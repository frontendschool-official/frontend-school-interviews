...
export default function ProblemCard({
  ...
  const getActionButtonClasses = (variant: 'primary' | 'secondary') => {
    const baseClasses = "px-4 py-3 rounded-lg font-semibold text-decoration-none transition-all duration-300 flex items-center gap-2 text-sm flex-1 justify-center";
  ...
  return (
    <div className="bg-secondary border border-border rounded-xl p-6 transition-all duration-300 h-full flex flex-col hover:-translate-y-1 hover:shadow-lg hover:border-primary">
      ...
      <div className="flex justify-between items-center mb-4 text-xs text-text/60">
        ...
        <span className={`px-3 py-1 rounded-lg font-semibold text-xs ${getDifficultyColor(problem.difficulty)}`}>
          {problem.difficulty}
        </span>
      </div>
      ...
      {showStatus && (
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-sm text-text/80">{getStatusText(problem.status)}</span>
        </div>
      )}
      ...
      {showScore && problem.score !== undefined && (
        <div className="text-center mb-4">
          <div className={`text-2xl font-bold ${getScoreColor(problem.score)} mb-2`}>
            {problem.score}%
          </div>
